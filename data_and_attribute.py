...
class Data():
    ...
    def __TransformAndCleansing(self):
        ...
        #13Des22 aslinya begini error format begitu -> data['order_date'] = pd.to_datetime(data['order_date'], format = '%d/%m/%Y')
        #yg ini ok -> data['order_date'] = pd.to_datetime(data['order_date'], format = '%m/%d/%Y')
        #error juga formatnya -> data['order_date'] = pd.to_datetime(data['order_date'], format='%d-%b-%y')
        data['order_date'] = pd.to_datetime(data['order_date'], format='%m/%d/%Y')
        #error format nya -> data['order_date'] = pd.to_datetime(data['order_date'], format='%m-%d-%y')
        #tdk berhasil juga -> data['order_date'] = pd.to_datetime(data['order_date'], format='%m/%d/%y')
        #error juga formatnya -> data['order_date'] = pd.to_datetime(data['order_date'], format = '%d %b %Y')        
        #wanna follow this example like in ETL programming -> df_participant['birth_date'] = pd.to_datetime(df_participant['birth_date'], format='%d %b %Y')
        data = data.drop(columns = ['provinsi', 'district'])

        #Penambahan kolom
        data['total_price'] = data['quantity'] * data['item_price']
        #bseditted 13Des22 data.insert(loc = 2, column = 'order_year' , value = data['order_date'].dt.year)
        data.insert(loc = 2, column = 'order_year' , value = data['order_date'].dt.year)
        #bseditted 13Des22 data.insert(loc = 2, column = 'order_month', value = data['order_date'].dt.month_name())
    ...
    def geoMaps(self, **parameter):
        os.path.dirname(os.path.realpath('__file__'))
        #bsedit11Jan2023 -> datageo = gpd.read_file('c:/bsapp/streamlit/gadm36_IDN_2.shp') 
        datageo = gpd.read_file('gadm36_IDN_2.shp') # --> you should move your directory to c:/bsapp/streamlit first!
